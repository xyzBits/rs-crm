...
impl AppConfig {
    pub fn load() -> Result<Self> {
        let dir = current_dir()?;
        info!("load crm-send app config current dir is {:?}", dir);
        let ret = match (
            File::open("crm-send/send.yml"), // main 函数运行时，根目录在 rs-crm
            File::open("send.yml"), // 单元测试运行时，根目录在 user-stat，直接在 idea 中运行，不是命令行中运行
...
#[test]
fn test_load_crm_send_config() -> Result<()> {
